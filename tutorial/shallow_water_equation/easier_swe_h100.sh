...
do
    ...
    if [ ! -f /mnt/local-fast/qpan/.easier/triangular_${n}.hdf5 ]; then
        echo "  Creating triangular mesh for n=${n}"
        python tutorial/create_triangular_mesh.py ${n} /mnt/local-fast/qpan/.easier/
    else
...
do
    ...
    if [ ! -f /mnt/local-fast/qpan/.easier/SW_${n}.hdf5 ]; then
        ...
        torchrun tutorial/shallow_water_equation/assemble_shallow_water.py \
            ...
            /mnt/local-fast/qpan/.easier/triangular_${n}.hdf5 /mnt/local-fast/qpan/.easier/SW_${n}.hdf5
    else
...
export OMP_NUM_THREADS=40 & export EASIER_DISABLE_JIT_HASH=1 & nsys profile -o easier  --stats=true  --trace=cuda,osrt,nvtx,openmp,mpi,cublas    --cudabacktrace=all  --force-overwrite=true  --delay 50 \
    torchrun tutorial/shallow_water_equation/swe_main.py \
        --profile=True --dt=0.0005 --backend=cuda \
        --device=cuda --comm_backend=nccl --output=res/ \
        /mnt/local-fast/qpan/.easier/triangular_2000.hdf5 /mnt/local-fast/qpan/.easier/SW_2000.hdf5  